...
The goal is to predict the IMDb rating of Indian movies using basic metadata such as year, duration, genre, and vote count. 
...
1. Data Loading 
Loaded the CSV file into a Pandas DataFrame using `pd.read_csv` with `latin1` encoding.
...
3. Feature Engineering
   - Used one-hot encoding (`pd.get_dummies`) to convert the `Genre` column into multiple binary features
     (e.g., `Genre_Action`, `Genre_Comedy`, etc.).
...
5. Model Evaluation
   ...
   This means the model’s predictions are on average ~1 rating point away from the true IMDb rating and
   it explains about 9% of the variation in ratings.

>Learning Outcomes

From this task I learned:
- How to clean messy real-world text data and convert it to numeric form.  
- How to handle missing values and prepare a clean training dataset.  
